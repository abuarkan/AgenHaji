...
@section('title', 'Dashboard Institusi B2B')
...
@section('sidebar-nav')
    ...
    <button onclick="switchTab('subagents')" class="flex items-center gap-2 px-4 py-2 text-xs font-bold rounded-lg transition-all cursor-pointer text-slate-600 hover:bg-slate-100/80 bg-white border border-slate-200" id="tab-btn-subagents">
        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
    ...
    <button onclick="switchTab('pending')" class="flex items-center justify-between gap-2 px-4 py-2 text-xs font-bold rounded-lg transition-all cursor-pointer text-slate-600 hover:bg-slate-100/80 bg-white border border-slate-200" id="tab-btn-pending">
        <div class="flex items-center gap-2">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path>
            </svg>
            <span>Persetujuan Agen</span>
        </div>
        @if($stats['pending_agents_count'] > 0)
            <span class="bg-red-500 text-white rounded-full px-1.5 py-0.5 text-[9px] font-black shrink-0 ml-1">{{ $stats['pending_agents_count'] }}</span>
        @endif
    </button>
...
@section('content')
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="space-y-6">
                ...
                <div class="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm flex items-center justify-between">
                    <div>
                        <span class="block text-[10px] font-bold text-slate-400 uppercase tracking-wider">Jemaah Kolektif</span>
                ...
                <!-- Sub Agents stats card -->
                <div class="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm">
                    <div class="flex items-center justify-between mb-4">
                        <div>
                            <span class="block text-[10px] font-bold text-slate-400 uppercase tracking-wider">Keanggotaan Institusi</span>
                            <span class="block text-2xl font-extrabold text-slate-800 mt-0.5">{{ $agent->institution->name ?? 'Institusi B2B' }}</span>
                        </div>
                    </div>
                    
                    <div class="space-y-2 text-xs font-semibold text-slate-600 border-t border-slate-100 pt-4">
                        <div class="flex justify-between">
                            <span>Sub-Agen Aktif:</span>
                            <strong class="text-slate-800">{{ $stats['sub_agents_count'] }} Agen</strong>
                        </div>
                        <div class="flex justify-between">
                            <span>Menunggu Persetujuan:</span>
                            <strong class="{{ $stats['pending_agents_count'] > 0 ? 'text-red-500' : 'text-slate-800' }}">
                                {{ $stats['pending_agents_count'] }} Agen
                            </strong>
                        </div>
                        <div class="flex justify-between">
                            <span>Total Komisi Institusi:</span>
                            <strong class="text-bpkh-navy">Rp {{ number_format($stats['total_commission'], 0, ',', '.') }}</strong>
                        </div>
                    </div>
                </div>

                <!-- Quick Help / Institutional Guide -->
                <div class="bg-gradient-to-br from-bpkh-navy to-slate-900 text-white rounded-2xl p-6 shadow-md border border-slate-800">
                    <span class="inline-block px-2 py-0.5 rounded bg-bpkh-gold text-slate-950 text-[9px] font-black uppercase mb-3">Portal Admin B2B</span>
                    <h4 class="text-xs font-bold uppercase tracking-wider mb-2">Persetujuan Mandiri</h4>
                    <p class="text-[11px] text-slate-300 leading-relaxed">
                        Sebagai Administrator Institusi, Anda memegang hak penuh untuk menyetujui sub-agen baru yang mendaftar menggunakan Kode Institusi Anda. Persetujuan ini menggantikan verifikasi eksternal oleh admin BPKH Pusat.
                    </p>
                </div>
            </div>
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <!-- Kinerja BPS-BPIH -->
                    <div class="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm">
                        <h4 class="text-xs font-bold text-slate-800 uppercase tracking-wider mb-6">Kinerja BPS BPIH Institusi</h4>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 items-center">
                            <div class="md:col-span-2 space-y-2">
                                <div class="flex items-center justify-between text-xs font-bold">
                                    <span class="{{ $isActive ? 'text-purple-200' : 'text-slate-550' }}">Progress Threshold Minimal ({{ $minTarget }} Porsi)</span>
    ...
    <div id="tab-jemaah" class="tab-panel hidden">
        ...
        <div class="bg-white border border-slate-200 rounded-2xl p-5 mb-6 shadow-sm">
            <div class="flex flex-col md:flex-row items-center justify-between gap-4 border-b border-slate-100 pb-5 mb-5">
                <h3 class="text-sm font-extrabold text-slate-800 uppercase tracking-wide">Kelola Jemaah Haji Institusi</h3>
            ...
            <!-- Filters Form -->
            <form action="{{ route('agent.institution') }}" method="GET" class="flex flex-col md:flex-row items-end gap-4">
                ...
                <div class="flex gap-2 w-full md:w-auto">
                    ...
                    @if(request()->anyFilled(['status', 'search_name', 'search_bank']))
                        <a href="{{ route('agent.institution') }}" class="px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-600 font-bold rounded-lg text-xs cursor-pointer transition-all border border-slate-200 text-center">
                            Reset
                        </a>
                    @endif
                </div>
        ...
        <div class="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
            <div class="overflow-x-auto">
                <table class="w-full text-xs text-left text-slate-600 min-w-[1050px] table-fixed">
                    <thead class="text-[10px] uppercase bg-slate-50 text-slate-500 border-b border-slate-200 font-bold tracking-wider">
                        <tr>
                            ...
                            <th class="px-6 py-4 w-24">Tipe Haji</th>
                            <th class="px-6 py-4 w-44">Didaftarkan Oleh</th>
                            <th class="px-6 py-4 w-40">Email</th>
                            <th class="px-6 py-4 w-44">Nomor Handphone</th>
                    ...
                    <tbody class="divide-y divide-slate-100">
                        @forelse($prospects as $prospect)
                            <tr class="group hover:bg-slate-50/80 transition-colors">
                                ...
                                <td class="px-6 py-4 break-words">
                                    <span class="block font-bold text-slate-800">{{ $prospect->agent->user->name ?? 'N/A' }}</span>
                                    <span class="block text-[10px] text-slate-400 font-medium">{{ $prospect->agent_id === $agent->id ? 'Institusi Admin' : 'Sub-Agen' }}</span>
                                </td>
                        ...
                        @empty
                            <tr>
                                <td colspan="10" class="px-6 py-8 text-center text-slate-400">Jemaah tidak ditemukan.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
    ...
    <!-- Tab 3: Daftar Sub-Agen -->
    <div id="tab-subagents" class="tab-panel hidden">
        <div class="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
            <div class="p-6 border-b border-slate-100 bg-slate-50/50">
                <h3 class="text-sm font-extrabold text-slate-800 uppercase tracking-wide">Kinerja & Daftar Sub-Agen B2B</h3>
                <p class="text-slate-500 text-xs mt-1">Daftar sub-agen aktif yang berada di bawah institusi Anda dan total jemaah yang didaftarkan.</p>
            </div>
            <div class="overflow-x-auto">
                <table class="w-full text-xs text-left text-slate-600">
                    <thead class="text-[10px] uppercase bg-slate-50 text-slate-500 border-b border-slate-200 font-bold tracking-wider">
                        <tr>
                            ...
                            <th class="px-6 py-4">Kinerja (Total Jemaah)</th>
                            <th class="px-6 py-4">Status Akun</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-slate-100">
                        @forelse($subAgents as $sa)
                            <tr class="hover:bg-slate-50/80 transition-colors">
                                <td class="px-6 py-4 font-bold text-slate-800">
                                    <a href="{{ route('agent.profile', $sa->referral_code) }}" class="text-bpkh-navy hover:underline">{{ $sa->user->name }}</a>
                                    <span class="block text-slate-400 text-[10px] font-normal mt-0.5">{{ $sa->user->email }}</span>
                                </td>
                                <td class="px-6 py-4 font-mono">
                                    <span class="block font-bold text-slate-700">NIP: {{ $sa->nip ?? '-' }}</span>
                                    <span class="block text-[10px] text-slate-400">NIK: {{ $sa->nik }}</span>
                                </td>
                                <td class="px-6 py-4 font-bold text-slate-700">{{ $sa->level->name }}</td>
                                <td class="px-6 py-4 font-mono text-slate-500">{{ $sa->whatsapp_number }}</td>
                                <td class="px-6 py-4">
                                    <span class="px-2.5 py-1 rounded-full bg-bpkh-navy/5 text-bpkh-navy border border-bpkh-navy/10 text-[10px] font-black uppercase">
                                        {{ $sa->prospects_count ?? 0 }} Jemaah
                                    </span>
                                </td>
                                <td class="px-6 py-4">
                                    @if($sa->status === 'active')
                                    ...
                                    @else
                                        <span class="px-2 py-0.5 rounded bg-red-50 text-red-700 border border-red-150 text-[10px] font-bold uppercase">Nonaktif</span>
                                    @endif
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="6" class="px-6 py-8 text-center text-slate-400">Tidak ada sub-agen aktif terdaftar.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
    ...
    <!-- Tab 4: Persetujuan Agen -->
    <div id="tab-pending" class="tab-panel hidden">
        <div class="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
            <div class="p-6 border-b border-slate-100 bg-slate-50/50">
                <h3 class="text-sm font-extrabold text-slate-800 uppercase tracking-wide">Persetujuan & Verifikasi Sub-Agen</h3>
                <p class="text-slate-500 text-xs mt-1">Daftar pendaftaran sub-agen baru yang memerlukan persetujuan administrator institusi.</p>
            </div>
            <div class="overflow-x-auto">
                <table class="w-full text-xs text-left text-slate-600">
                    <thead class="text-[10px] uppercase bg-slate-50 text-slate-500 border-b border-slate-200 font-bold tracking-wider">
                        <tr>
                            <th class="px-6 py-4">Detail Pelamar</th>
                            <th class="px-6 py-4">Dokumen Identitas</th>
                            <th class="px-6 py-4">Dokumen Pendukung Pekerja</th>
                            <th class="px-6 py-4">Tanggal Pengajuan</th>
                            <th class="px-6 py-4 text-center">Tindakan</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-slate-100">
                        @forelse($pendingAgents as $pa)
                            <tr class="hover:bg-slate-50/80 transition-colors">
                                <td class="px-6 py-4 font-bold text-slate-800">
                                    {{ $pa->user->name }}
                                    <span class="block text-slate-400 text-[10px] font-normal mt-0.5">{{ $pa->user->email }}</span>
                                    <span class="block text-[10px] text-slate-500 font-bold mt-1">NIP: {{ $pa->nip ?? '-' }}</span>
                                    <span class="block text-[10px] text-slate-500 font-mono">NIK: {{ $pa->nik }}</span>
                                </td>
                                <td class="px-6 py-4">
                                    @if($pa->ktp_photo_path)
                                        <a href="{{ asset('storage/' . $pa->ktp_photo_path) }}" target="_blank" class="text-bpkh-navy hover:underline font-bold">Lihat Foto KTP</a>
                                    @else
                                        <span class="text-slate-400">-</span>
                                    @endif
                                </td>
                                <td class="px-6 py-4">
                                    @if($pa->id_card_employee_path)
                                        <a href="{{ asset('storage/' . $pa->id_card_employee_path) }}" target="_blank" class="text-bpkh-navy hover:underline font-bold">Lihat ID Card / SK</a>
                                    @else
                                        <span class="text-slate-400">-</span>
                                    @endif
                                </td>
                                <td class="px-6 py-4 font-mono text-slate-400">{{ $pa->updated_at->format('d/m/Y H:i') }}</td>
                                <td class="px-6 py-4 text-center">
                                    <div class="flex items-center justify-center gap-2">
                                        <form action="{{ route('agent.institution.verify', $pa->id) }}" method="POST" class="inline">
                                            @csrf
                                            <input type="hidden" name="action" value="approve">
                                            <button type="submit" class="px-3 py-1.5 bg-emerald-50 hover:bg-emerald-100 text-emerald-700 font-extrabold rounded text-[10px] cursor-pointer border border-emerald-250 transition-all">
                                                Setujui
                                            </button>
                                        </form>
                                        <form action="{{ route('agent.institution.verify', $pa->id) }}" method="POST" class="inline">
                                            ...
                                            <button type="submit" class="px-3 py-1.5 bg-red-50 hover:bg-red-100 text-red-700 font-extrabold rounded text-[10px] cursor-pointer border border-red-250 transition-all">
                                                Tolak
                                            </button>
                                        </form>
                                    </div>
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="px-6 py-8 text-center text-slate-400 font-medium">Tidak ada permohonan persetujuan sub-agen baru.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
    ...
    <!-- Tab 5: Buku Besar Komisi -->
    <div id="tab-ledger" class="tab-panel hidden">
        <div class="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
            <div class="p-6 border-b border-slate-100 bg-slate-50/50">
                <h3 class="text-sm font-extrabold text-slate-800 uppercase tracking-wide">Buku Besar & Riwayat Mutasi Komisi B2B</h3>
                <p class="text-slate-500 text-xs mt-1">Daftar lengkap perolehan komisi dan riwayat pencairan saldo institusi.</p>
            </div>
            <div class="overflow-x-auto">
                <table class="w-full text-xs text-left text-slate-600">
                    <thead class="text-[10px] uppercase bg-slate-50 text-slate-500 border-b border-slate-200 font-bold tracking-wider">
    ...
    <script>
        function switchTab(tabId) {
            ...
            const buttons = {
                ...
                'subagents': 'tab-btn-subagents',
                'pending': 'tab-btn-pending',
